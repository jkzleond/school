...
{
	function get_tree_navigation($items, $id = NULL, $class = NULL, $first_class = NULL, $last_class = NULL, $depth = 0)
	{
		// HTML Attributes
		$id = ( ! is_null($id) ) ? ' id="' . $id . '" ' : '';
		...
		foreach($items as $key => $page)
		{
			if ($key !== 'articles')
			{
                ...
                {
                    $tree .= '<a'.$class.' href="' . (($page['has_url'] != 0) ? $page['absolute_url'] : '#') . '">'.$title. '</a>';

                }
				

				if (!empty($page['children']))
					$tree .= get_tree_navigation($page['children'], NULL, NULL, NULL, NULL, $depth+1);
				...
				if (!empty($page['articles']))
				{
					...
					foreach($page['articles'] as $article)
					{
						...
						$tree .= '<a'.$class.' href="' . $article['url'] . '">'.$article['title']. '</a>';
					}
					$tree .= '</span>';
				}
            ...
            if ($key !== count($items) - 1 and $depth === 0)
            {
                $tree .= '<li class="menu_line">';
            }
